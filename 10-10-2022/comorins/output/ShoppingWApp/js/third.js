...
function disp(){                                                //diplay the price list  and diaplay the carts
    ...
    let logins=JSON.parse(localStorage.getItem("logins"));
     if(!logers&&!logers[d].carts[0]){
        document.getElementById("body").innerHTML=`<div class='col-12 pob'><a href="index.html" onclick=""><button class='float-right bg-primary po'>Go to Shopping</button></a></div>`
    ...
    for(let i=0; i<list[d].carts.length;i++){
            ...
            total=Math.round(tp+dc+pkf-dis);
